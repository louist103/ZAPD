...
{
	ZRoom* room = static_cast<ZRoom*>(res);
	...
	for (size_t i = 0; i < room->commands.size(); i++)
	{
		...
		writer->Write(static_cast<uint8_t>(cmd->cmdID));

		switch (cmd->cmdID)
		{
		case RoomCommand::SetWind:
		{
			SetWind* cmdSetWind = static_cast<SetWind*>(cmd);

			writer->Write(static_cast<uint8_t>(0));  // 0x01
			writer->Write(static_cast<uint8_t>(0));  // 0x02
			writer->Write(static_cast<uint8_t>(0));  // 0x03
		...
		{
			SetTimeSettings* cmdTime = static_cast<SetTimeSettings*>(cmd);
			...
			writer->Write(static_cast<uint8_t>(0));  // 0x07
		}
		break;
		case RoomCommand::SetSkyboxModifier:
		{
			SetSkyboxModifier* cmdSkybox = static_cast<SetSkyboxModifier*>(cmd);
			...
			writer->Write(static_cast<uint8_t>(0));    // 0x06
			writer->Write(static_cast<uint8_t>(0));    // 0x07
		}
		break;
		case RoomCommand::SetEchoSettings:
		{
			SetEchoSettings* cmdEcho = static_cast<SetEchoSettings*>(cmd);

			writer->Write(static_cast<uint8_t>(0));              // 0x01
			writer->Write(static_cast<uint8_t>(0));              // 0x02
			writer->Write(static_cast<uint8_t>(0));              // 0x03
			writer->Write(static_cast<uint8_t>(0));              // 0x04
			writer->Write(static_cast<uint8_t>(0));              // 0x05
			writer->Write(static_cast<uint8_t>(0));              // 0x06
			writer->Write(static_cast<uint8_t>(cmdEcho->echo));  // 0x07
		}
		break;
		case RoomCommand::SetSoundSettings:
		{
			SetSoundSettings* cmdSound = static_cast<SetSoundSettings*>(cmd);

			writer->Write(static_cast<uint8_t>(cmdSound->reverb));  // 0x01
			writer->Write(static_cast<uint8_t>(0));                 // 0x02
			writer->Write(static_cast<uint8_t>(0));                 // 0x03
			writer->Write(static_cast<uint8_t>(0));                 // 0x04
			writer->Write(static_cast<uint8_t>(0));                 // 0x05
		...
		{
			SetSkyboxSettings* cmdSkybox = static_cast<SetSkyboxSettings*>(cmd);

			writer->Write(static_cast<uint8_t>(cmdSkybox->unk1));          // 0x01
			writer->Write(static_cast<uint8_t>(0));                        // 0x02
			writer->Write(static_cast<uint8_t>(0));                        // 0x03
			writer->Write(static_cast<uint8_t>(cmdSkybox->skyboxNumber));  // 0x04
			writer->Write(static_cast<uint8_t>(cmdSkybox->cloudsType));    // 0x05
			writer->Write(static_cast<uint8_t>(cmdSkybox->isIndoors));     // 0x06
		}
		break;
		case RoomCommand::SetRoomBehavior:
		{
			SetRoomBehavior* cmdRoom = static_cast<SetRoomBehavior*>(cmd);

			writer->Write(static_cast<uint8_t>(cmdRoom->gameplayFlags));  // 0x01
			writer->Write(static_cast<uint8_t>(0));                       // 0x02
			writer->Write(static_cast<uint8_t>(0));                       // 0x03
		...
		{
			SetCsCamera* cmdCsCam = static_cast<SetCsCamera*>(cmd);

			writer->Write(static_cast<uint8_t>(cmdCsCam->cameras.size()));  // 0x01
			writer->Write(static_cast<uint8_t>(0));                         // 0x02
			writer->Write(static_cast<uint8_t>(0));                         // 0x03
		...
		{
			SetMesh* cmdMesh = static_cast<SetMesh*>(cmd);

			int baseStreamEnd = writer->GetStream().get()->GetLength();

			writer->Write(static_cast<uint8_t>(cmdMesh->data));  // 0x01
			writer->Write(static_cast<uint8_t>(0));              // 0x02
			writer->Write(static_cast<uint8_t>(0));              // 0x03
		...
		{
			SetCameraSettings* cmdCam = static_cast<SetCameraSettings*>(cmd);

			writer->Write(static_cast<uint8_t>(cmdCam->cameraMovement));  // 0x01
			writer->Write(static_cast<uint8_t>(0));                       // 0x02
			writer->Write(static_cast<uint8_t>(0));                       // 0x03
		...
		{
			SetLightingSettings* cmdLight = static_cast<SetLightingSettings*>(cmd);

			writer->Write(static_cast<uint8_t>(cmdLight->settings.size()));  // 0x01
			writer->Write(static_cast<uint8_t>(0));                          // 0x02
			writer->Write(static_cast<uint8_t>(0));                          // 0x03
		...
		{
			SetRoomList* cmdRoom = static_cast<SetRoomList*>(cmd);

			writer->Write(static_cast<uint8_t>(cmdRoom->rooms.size()));  // 0x01
			writer->Write(static_cast<uint8_t>(0));                      // 0x02
			writer->Write(static_cast<uint8_t>(0));                      // 0x03
		...
		{
			SetCollisionHeader* cmdCollHeader = static_cast<SetCollisionHeader*>(cmd);
		...
		{
			SetEntranceList* cmdEntrance = static_cast<SetEntranceList*>(cmd);
			...
			{
				writer->Write(static_cast<uint8_t>(entry.startPositionIndex));
				writer->Write(static_cast<uint8_t>(entry.roomToLoad));
			}
		...
		{
			SetSpecialObjects* cmdSpecObj = static_cast<SetSpecialObjects*>(cmd);

			writer->Write(static_cast<uint8_t>(cmdSpecObj->elfMessage));     // 0x01
			writer->Write(static_cast<uint8_t>(0));                          // 0x02
			writer->Write(static_cast<uint8_t>(0));                          // 0x03
			writer->Write(static_cast<uint8_t>(0));                          // 0x04
			writer->Write(static_cast<uint8_t>(0));                          // 0x05
			writer->Write(static_cast<uint16_t>(cmdSpecObj->globalObject));  // 0x06
		}
		break;
		case RoomCommand::SetStartPositionList:
		{
			SetStartPositionList* cmdStartPos = static_cast<SetStartPositionList*>(cmd);

			uint32_t baseStreamEnd = writer->GetStream().get()->GetLength();

			writer->Write(static_cast<uint8_t>(cmdStartPos->actors.size()));  // 0x01
			writer->Write(static_cast<uint8_t>(0));                           // 0x02
			writer->Write(static_cast<uint8_t>(0));                           // 0x03
			writer->Write(baseStreamEnd);                                     // 0x04
